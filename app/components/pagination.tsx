...
const ProfessionalExperience = () => {
    ...
    return (
        <div id='professional' className="relative flex flex-col items-center pr-16 pl-16 w-full bg-cover bg-center min-h-[738px] rounded-[60px] max-md:px-5 py-10 md:py-24 max-md:max-w-full" style={{ backgroundImage: 'url("/My Services.png")' }}>
            ...
            {/* Show all 8 items on mobile and paginated 4 items on desktop */}
            <div className="relative flex z-0 flex-wrap gap-8 items-center mt-24 text-white max-md:flex-wrap max-md:justify-center max-md:mt-10 h-full max-md:max-w-full transition-all duration-500 ease-in-out transform">
                {/* On desktop, show paginated 4 items */}
                <div className="hidden md:grid grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 max-md:mt-10 max-w-[1440px] h-full">
                    {experiences
                        ...
                        .map((exp, idx) => (
                            ...
                            >
                                <div className="self-start ml-6 text-xl font-medium max-md:ml-2.5">
                                    {exp.company}
                                </div>
                                <div className="shrink-0 mt-6 h-px border border-solid border-white border-opacity-50" />
                                <div className="flex flex-col px-6 mt-6 text-base leading-6 max-md:px-5">
                ...
                <div className="hidden max-md:flex flex-wrap gap-8 justify-center items-center">
                    {experiences.map((exp, idx) => (
                        <div key={idx} className="flex flex-col self-stretch py-7 my-auto rounded-3xl border border-white border-solid backdrop-blur-[7.5px] bg-white bg-opacity-10 min-w-[240px] w-[298px] transition-all duration-500 ease-in-out">
                            <div className="self-start ml-6 text-xl font-medium max-md:ml-2.5">
                                {exp.company}
                            </div>
                            <div className="shrink-0 mt-6 h-px border border-solid border-white border-opacity-50" />
            ...
            {/* Pagination Dots in the Center (hidden on mobile) */}
            <div className="hidden md:flex absolute bottom-10 left-1/2 transform -translate-x-1/2 gap-2 items-center z-0 px-4 py-2.5 rounded-xl border border-solid bg-white bg-opacity-10 border-white border-opacity-10">
            ...
            <div className="hidden md:flex absolute right-10 top-2/3 transform -translate-y-1/2 z-50 flex gap-2">
                {currentIndex + itemsPerPage < experiences.length && (
                    <button
                        onClick={handleNext}
                        className="bg-[#00A5B6] text-white rounded-full w-12 h-12 flex items-center justify-center"
                    >
                        <img
                            loading="lazy"
                            src="/arrow-right.png"
            ...
            {/* Left Arrow Button (only visible after scrolling, hidden on mobile) */}
            {currentIndex > 0 && (
                <div className="hidden md:flex absolute left-10 top-2/3 transform -translate-y-1/2 z-50">
                    <button
                        onClick={handlePrev}
                        className="bg-[#00A5B6] text-white rounded-full w-12 h-12 flex items-center justify-center"
                    >
